...
| `tailwind-4-docs` | **Its docs snapshot is not initialised in this checkout** — `.agents/skills/tailwind-4-docs/references/` does not exist, and the skill's own instruction is to refuse to answer until the snapshot is synced. It is therefore not a verified source here (`AGENTS.md` §9 rule 2). Tailwind 4 questions are answered from the `tailwind-design-system` skill and from Tailwind's live docs instead |
